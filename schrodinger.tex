    ...
    \begin{frame}{Elektronen-Wellenfunktion im Grundzustand}
        ...
        \begin{tikzpicture}
            \begin{axis}
                ...
                width=0.8\textwidth,
                height=6cm,
                colormap/viridis,    % Moderner Farbverlauf
                view = {45}{30},     % Perspektive (Azimut, Elevation)
                xlabel = $x$,
                ylabel = $y$,
                ...
                title = {Quantenmechanisches Elektronenorbital},
                ...
                surf,
                shader = interp,
                samples = 50,        % Ausreichend für glatte Darstellung
                domain = -3:3,       % Definitionsbereich
                y domain = -3:3,
                ]
                    {exp(-(x^2 + y^2))};     % Gaußsches Wellenpaket (vereinfacht)

                \node[white,font=\tiny,rotate=90] at (axis cs:-3.5,-3.5,0.8)
                    {Maximale Aufenthaltswahrscheinlichkeit im Zentrum};
            \end{axis}
        \end{tikzpicture}
    \end{frame}
...
%    \end{frame}
    ...
    \begin{frame}{Lösung: High-K Materialien}
        \begin{itemize}
            \item Siliziumdioxid (SiO₂) wird durch High-K-Materialien ersetzt.
            \item Beispiel: Hafniumdioxid (HfO₂) – höhere Barrierenhöhe reduziert das Tunneln.
